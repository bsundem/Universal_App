"""
Finance service for financial calculations and analysis.

This service provides financial calculations using R integration, including
yield curve analysis, option pricing, and portfolio metrics.
"""
import os
import tempfile
...
# Get service logger
logger = ServiceLogger("finance_service")


class FinanceService:
    """Service for financial calculations and analysis using R integration."""
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        Args:
            data_dir: Directory to store finance data, defaults to 'data/finance'
        """
    ...
    def calculate_yield_curve(
        ...
        try:
            ...
            logger.info(f"Yield curve calculation successful, returning {len(df)} rows")
            return df
            
        except Exception as e:
            logger.error(f"Failed to calculate yield curve: {str(e)}", "calculate_yield_curve")
            raise ServiceError(
    ...
    def price_option(
        ...
        try:
            ...
            logger.info(f"Option pricing successful: {result}")
            return result
            
        except Exception as e:
            logger.error(f"Failed to price option: {str(e)}", "price_option")
            
            # Fall back to calculating with Python if R fails
            try:
                logger.info("Falling back to Python implementation", "price_option")
    ...
    def calculate_portfolio_metrics(
        ...
        try:
            ...
            logger.info(f"Portfolio metrics calculation successful: {result}")
            return result
            
        except Exception as e:
            logger.error(f"Failed to calculate portfolio metrics: {str(e)}", "calculate_portfolio_metrics")
            
            # Fall back to Python implementation
            try:
                logger.info("Falling back to Python implementation", "calculate_portfolio_metrics")
